...
- `review`: 客观审阅指定 session folder 中的 checkpoint 及相关工作状态, 默认不恢复上下文, 不继续实现.
...
`review` 用于客观审阅指定 session folder 中的 checkpoint 及相关工作状态, 默认不恢复上下文, 不继续实现.
...
- 然后审阅被引用的工作内容是否存在缺陷, 边界问题, 遗漏验证, 冲突或不一致.
