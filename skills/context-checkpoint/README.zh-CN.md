...
`context-checkpoint` 是一个 agent 无关的上下文管理 skill, 用于在长任务或多会话工作中保存和重建任务上下文.
...
- `restore`: 从当前会话 checkpoint 或指定 session folder 中的 checkpoint 重建任务上下文.
...
`restore` 用于从当前会话 checkpoint 或显式指定的 session folder 中的 checkpoint 重建任务上下文.
...
- 当项目文件和 checkpoint 内容冲突时, 优先相信当前项目文件.
- restore 默认只读, 且不会复制 checkpoint 文件, 除非用户明确要求后续工作.
