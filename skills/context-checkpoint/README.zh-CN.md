...
`SKILL.md` 是轻量路由层, 只保留命令选择规则, 全局不变量和语言规则. 每个命令在需要时按需加载 `references/` 下的对应 reference, 所有 checkpoint 文件结构集中在 `references/file-contracts.md`. 这种渐进式披露结构让常驻 skill 保持精简, 同时让每个命令都能独立执行.
...
- `review`: 客观审阅指定 session folder 中的 checkpoint 所指向的实际工作内容, 再将结果写入该 folder 的 `REVIEW.md`, 不恢复上下文, 不继续实现.

## Workflow

`context-checkpoint` 围绕 session folder 组织. session folder 是上下文身份, agent 只是按所运行命令读取或写入该上下文的协作者.

常见工作流:

- Single session workflow: 单个会话工作, 运行 `update`, 之后运行 `restore`, 再继续工作并再次 update.
- Shared session collaboration workflow: 多个 agent 或多个会话使用同一个 session folder 作为共享上下文主线. 每个协作者对该 folder 运行 `restore`, 继续工作, 再运行 `update` 把新状态写回.
- Review feedback workflow: 审阅者对 source session folder 运行 `review`. 结果会写入该 folder 的 `REVIEW.md`. 后续对同一 folder 运行 `restore` 时, 会把这些 findings 浮出为需要重新核验的 open issues.
- Handoff or branching workflow: 一个 session folder 作为只读 source, 当前 session folder 作为可写 target. 当上下文需要迁移或派生到不同 session folder 时使用这个工作流.
- Explicit restore conflict guard: 如果当前会话已经有 checkpoint 文件, 又尝试 `restore` 另一个 session folder, `restore` 会停止, 避免混合两条上下文主线. 需要迁移时使用 `handoff`, 需要协作时共享同一个 session folder.
...
- `restore`: 只读. 默认不复制 checkpoint 文件, 不写会话文件夹, 不修改项目文件, 不执行 TODO. 当 source session folder 存在 `REVIEW.md` 时, 会把其中的 findings 作为待重新核验的 open issues 浮出.
- `handoff`: source session folder 只读. target session folder 可读写. 只能复制 source session folder 内的 non-checkpoint artifacts.
- `review`: 读取 checkpoint 和项目文件, 只可写 source session folder 中的 `REVIEW.md`. 不修改 source checkpoint 文件, 不修改项目文件, 不执行 TODO.
...
- 当 source session folder 存在 `REVIEW.md` 时, 会把其中的 findings 作为待重新核验的 open issues 浮出, 并说明这些结论反映审阅时刻的工作状态, 需对照当前项目文件重新核验.
...
`review` 用于客观审阅指定 session folder 中的 checkpoint 所指向的实际工作内容, 再将结果写入该 folder 的 `REVIEW.md`, 不恢复上下文, 不继续实现.
...
- review 结果会写入 source session folder 的 `REVIEW.md`, 覆盖此前的 `REVIEW.md`, 同时在回复中输出.
- 除写入 `REVIEW.md` 外, review 不修改 source checkpoint 文件, 不修改项目文件, 不执行 TODO.

Review 输出和 `REVIEW.md` 使用以下章节:
...
`REVIEW.md` 文件会在这些章节之前额外记录 `Reviewed Session` 和 `Review Date` 抬头.
