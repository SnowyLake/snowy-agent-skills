...
Resolve the source session folder:
...
- Must not modify source `CONTEXT.md`, source `HISTORY.md`, other source-folder artifacts, or project files unless the user explicitly asks for follow-up work.
...
9. Build the review scope from `Current Goal`, `Work Artifacts`, `Relevant Files`, `TODO`, `Next Actions`, and any source-folder artifacts.
...
16. Write the review result to `REVIEW.md` in the source session folder, overwriting any existing `REVIEW.md`, and also output it in the response.

## Output Style

Write the result to `REVIEW.md` in the source session folder and output the same result in the response. Use these sections in both:
